...
def simplify_note():
    ...
    try:
        ...
        if not note_id:
            return jsonify({
                'success': False,
                'error': 'Note ID is required. If you want to test with sample text, provide note_text instead.'
            }), 400
        ...
        if not output:
            # Try to parse raw output with improved parsing
            raw_output = result.get("simplified_output", "")
            ...
            try:
                ...
                # If still no JSON, try to extract structured information
                if output is None:
                    output = parse_text_to_structure(cleaned)
                    output["_raw_output"] = raw_output
